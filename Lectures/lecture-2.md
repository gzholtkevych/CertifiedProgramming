...
>Помічені внутрішні вузли цього дерева відповідають конструванню абстракції за змінною, яка є міткою вузла.<br/>
>Непомічені внутрішні вузли цього дерева відповідають конструктору застосування.<br/>
>Листя дерева зображені прямокутниками з заокругленими кутами; вони відповідають конструюванню атома за змінною, яка є міткою вузла.
...
Припустимо, що терм $(\uplambda\ x\mathop{.}(x x))$ можна типізувати, тоді його тип можна отримати тільки за правилом абстракції, а тому він має вид
$\alpha\rightarrow\beta$:

```math
\vdash(\uplambda\ x\mathop{.}(x x)):\alpha\rightarrow\beta.
```

Але правило абстракції, щоб отримати таке судження, можна застосувати тільки до суджень виду

```math
x:\alpha\vdash(x x):\beta.
```

Судження такого виду можна отримати тільки за рахунок використання правила застосування, а тоді тип $x$ повинен мати вид $\gamma\rightarrow\beta$,
співпадаячи як з $\gamma$, так і з $\alpha$.
Інакше кажучи маємо систему рівнянь в типах

```math
\begin{cases}
\gamma\rightarrow\beta=\gamma \\
\gamma\rightarrow\beta=\alpha
\end{cases}
```

Якщо цю систему можна розв'язати, тоді (в сиду першого рівняння) має бути вірною рівність

```math
\max(\mathrm{ord}(\gamma)+1,\mathrm{ord}(\beta))=\mathrm{ord}(\gamma)
```

Проте ліва частина цієї рівності принаймні на одиницю більша від правої, тобто припущення про типізуємість є хибним.

### Властивості простої типізації

**Замкненість.**
При конструюванні термів з уже типізованих отримуємо типізуємий терм.

**Стійкість відносно перейменування.**
При перейменуванні змінної у типізоваому термі, терм залишається типізуємим з тим самим типом.

**Типова безпека.**
При редукції типізованого терму, він залишається типізуємим з тим самим типом.

**Сильна нормалізуємість.**
Кожний типізуємий терм є сильно нормалізуємим.
